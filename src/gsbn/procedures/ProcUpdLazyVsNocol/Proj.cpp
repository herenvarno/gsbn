...
void Proj::init_new(ProcParam proc_param, ProjParam proj_param, Database &db, vector<Proj *> *list_proj, vector<Pop *> *list_pop)
{
	...
	CHECK(_pij_nocol = db.create_sync_vector_f32("pijnocol_" + to_string(_id)));
	CHECK(_eij_nocol = db.create_sync_vector_f32("eijnocol_" + to_string(_id)));
	CHECK(_zi2_nocol = db.create_sync_vector_f32("zi2nocol_" + to_string(_id)));
	CHECK(_zj2_nocol = db.create_sync_vector_f32("zj2nocol_" + to_string(_id)));
	CHECK(_tij = db.create_sync_vector_i32("tij_" + to_string(_id)));
	CHECK(_wij = db.create_sync_vector_f32("wij_" + to_string(_id)));
	CHECK(_wij_nocol = db.create_sync_vector_f32("wijnocol_" + to_string(_id)));
	...
	_pij_nocol->mutable_cpu_vector()->resize(_dim_hcu * _dim_conn * _dim_mcu, 1.0 / _dim_conn / _dim_mcu);
	_eij_nocol->mutable_cpu_vector()->resize(_dim_hcu * _dim_conn * _dim_mcu);
	_zi2_nocol->mutable_cpu_vector()->resize(_dim_hcu * _dim_conn * _dim_mcu);
	_zj2_nocol->mutable_cpu_vector()->resize(_dim_hcu * _dim_conn * _dim_mcu);
	_tij->mutable_cpu_vector()->resize(_dim_hcu * _dim_conn * _dim_mcu);
	_wij->mutable_cpu_vector()->resize(_dim_hcu * _dim_conn * _dim_mcu);
	_wij_nocol->mutable_cpu_vector()->resize(_dim_hcu * _dim_conn * _dim_mcu);
		...
		if (pdt > 0)
		{
			...
			pi = (pi - ((ei * kp * kzi - ei * ke * kp + ke * kp * zi) / (ke - kp) +
					 ...
					 ((exp(kp * pdt - ke * pdt) * (ei * kp * kzi - ei * ke * kp + ke * kp * zi)) / (ke - kp) +
						(ke * kp * zi * exp(kp * pdt - kzi * pdt)) / (kp - kzi)) /
							 (exp(kp * pdt) * (ke - kzi));
...
void Proj::update_row_cpu()
{
	...
	float *ptr_zi2 = _zi2->mutable_cpu_data();
	float *ptr_zj2 = _zj2->mutable_cpu_data();
	float *ptr_pij_nocol = _pij_nocol->mutable_cpu_data();
	float *ptr_eij_nocol = _eij_nocol->mutable_cpu_data();
	float *ptr_zi2_nocol = _zi2_nocol->mutable_cpu_data();
