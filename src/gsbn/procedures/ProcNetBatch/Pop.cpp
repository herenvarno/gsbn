...
void Pop::init_new(SolverParam solver_param, PopParam pop_param, Database& db, vector<Pop*>* list_pop, int *hcu_cnt, int *mcu_cnt, Msg *msg){
	CHECK(list_pop);
	_list_pop=list_pop;
	CHECK(msg);
	_msg=msg;
...
/*
	for(map<int, vector<int>>::iterator it=_ext_spikes.begin(); it!=_ext_spikes.end(); it++){
		cout << it->first << ":";
...
//		LOG(INFO) << "i=" << i << ",j="<< j<<",bj="<< ptr_bj[offset+idx] << ",epsc="<<ptr_epsc[offset+idx];
...
void Pop::set_from_ext_spike_cpu(){
	if(!_flag_ext_spike){
		return;
	}
