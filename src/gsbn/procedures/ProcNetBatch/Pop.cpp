...
void Pop::init_copy(PopParam pop_param, Database& db, vector<Pop*>* list_pop, int *hcu_cnt, int *mcu_cnt, Msg *msg){
	...
	*mcu_cnt += _dim_hcu * _dim_mcu;
	...
	CHECK(_slot=db.sync_vector_i("slot_"+to_string(_id)));
	CHECK(_fanout=db.sync_vector_i("fanout_"+to_string(_id)));	
	...
	CHECK(_spike = db.sync_vector_i("spike_"+to_string(_id)));
	CHECK(_rnd_uniform01 = db.create_sync_vector_f(".rnd_uniform01"+to_string(_id)));
	CHECK(_rnd_normal = db.create_sync_vector_f(".rnd_normal"+to_string(_id)));
	CHECK(_wmask = db.sync_vector_f(".wmask"));
	CHECK(_lginp = db.sync_vector_f(".lginp"));
