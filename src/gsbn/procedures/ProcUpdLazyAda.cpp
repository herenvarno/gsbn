#include "gsbn/procedures/ProcUpdLazyAda.hpp"

namespace gsbn{
namespace proc_upd_lazy_ada{

REGISTERIMPL(ProcUpdLazyAda);

void ProcUpdLazyAda::init_new(SolverParam solver_param, Database& db){
...
void ProcUpdLazyAda::update_cpu(){
	int cycle_flag;
...
void ProcUpdLazyAda::update_gpu(){
	int cycle_flag;
	CHECK(_glv.geti("cycle-flag", cycle_flag));
	if(cycle_flag != 1){
		return;
