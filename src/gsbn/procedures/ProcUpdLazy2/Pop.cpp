...
void Pop::init_new(int max_cycle, ProcParam proc_param, PopParam pop_param, Database& db, vector<Pop*>* list_pop, int *hcu_cnt, int *mcu_cnt, Msg *msg){
	CHECK(list_pop);
	_list_pop=list_pop;
	CHECK(msg);
	_msg=msg;
	...
	*mcu_cnt += _dim_hcu * _dim_mcu;
	...
	_spike->mutable_cpu_vector()->resize(max_cycle * _dim_hcu * _dim_mcu);
	_rnd_uniform01->mutable_cpu_vector()->resize(max_cycle * _dim_hcu * _dim_mcu);
	_rnd_normal->mutable_cpu_vector()->resize(max_cycle * _dim_hcu * _dim_mcu);
...
void Pop::init_copy(int max_cycle, ProcParam proc_param, PopParam pop_param, Database& db, vector<Pop*>* list_pop, int *hcu_cnt, int *mcu_cnt, Msg *msg){
	...
	*mcu_cnt += _dim_hcu * _dim_mcu;
	...
	_act->mutable_cpu_vector()->resize(_dim_hcu * _dim_mcu);
	CHECK_EQ(_spike->cpu_vector()->size(), max_cycle * _dim_hcu * _dim_mcu);
	_rnd_uniform01->mutable_cpu_vector()->resize(max_cycle * _dim_hcu * _dim_mcu);
	_rnd_normal->mutable_cpu_vector()->resize(max_cycle * _dim_hcu * _dim_mcu);
