...
void Proj::init_new(SolverParam solver_param, ProjParam proj_param, Database& db, vector<Proj*>* list_proj, vector<Pop*>* list_pop, Msg *msg, int norm_frac_bit, int p_frac_bit){
	...
	CHECK(_pi = db.create_sync_vector_i32("pi_"+to_string(_id)));
	...
	CHECK(_pj = db.create_sync_vector_i32("pj_"+to_string(_id)));
	CHECK(_ej = db.create_sync_vector_i16("ej_"+to_string(_id)));
	CHECK(_zj = db.create_sync_vector_i16("zj_"+to_string(_id)));
	CHECK(_pij = db.create_sync_vector_i32("pij_"+to_string(_id)));
...
/*	int i=15;
	...
	_pj_frac_bit = i-1;
	_pij_frac_bit = i-1;
	LOG(INFO) << _pi_frac_bit << ", " << _pj_frac_bit << ", " << _pij_frac_bit;
//	exit(0);*/

	_pi_frac_bit = p_frac_bit;
	_pj_frac_bit = p_frac_bit;
	_pij_frac_bit = p_frac_bit;
	
	_ii->resize(_dim_hcu * _dim_conn, -1);
	_qi->resize(_dim_hcu * _dim_conn);
	_di->resize(_dim_hcu * _dim_conn);
	_pi->resize(_dim_hcu * _dim_conn, fp32_to_fix32(pi0, _pi_frac_bit));
	_ei->resize(_dim_hcu * _dim_conn);
	_zi->resize(_dim_hcu * _dim_conn);
	_ti->resize(_dim_hcu * _dim_conn);
	_pj->resize(_dim_hcu * _dim_mcu, fp32_to_fix32(pj0, _pj_frac_bit));
	_ej->resize(_dim_hcu * _dim_mcu);
	_zj->resize(_dim_hcu * _dim_mcu);
	_pij->resize(_dim_hcu * _dim_conn * _dim_mcu, fp32_to_fix32(pij0, _pij_frac_bit));
...
void Proj::init_copy(ProjParam proj_param, Database& db, vector<Proj*>* list_proj, vector<Pop*>* list_pop, Msg *msg, int norm_frac_bit, int p_frac_bit){
	...
	CHECK(_ssi = db.create_sync_vector_i32(".ssi_"+to_string(_id)));
	CHECK(_pi = db.sync_vector_i32("pi_"+to_string(_id)));
	...
	CHECK(_ssj = db.create_sync_vector_i32(".ssj_"+to_string(_id)));
	CHECK(_pj = db.sync_vector_i32("pj_"+to_string(_id)));
	CHECK(_ej = db.sync_vector_i16("ej_"+to_string(_id)));
	CHECK(_zj = db.sync_vector_i16("zj_"+to_string(_id)));
	CHECK(_pij = db.sync_vector_i32("pij_"+to_string(_id)));
	...
	float pij0 = pi0/_dim_mcu;
	int i=15;
	while(pow(2, 15-i)>pi0){
		i++;
	}
	_pi_frac_bit = i-1;
	i=15;
	while(pow(2, 15-i)>pj0){
		i++;
	}
	_pj_frac_bit = i-1;
	i=15;
	while(pow(2, 15-i)>pij0){
		i++;
	}
	_pij_frac_bit = i-1;
}
...
void update_full_kernel_cpu(
	...
	if(j==0){
		float pi = fix32_to_fp32(ptr_pi[i], pi_frac_bit);
		...
		if(pdt<=0){
			...
			ptr_pi[i] = fp32_to_fix32(pi, pi_frac_bit);
	...
	if(pdt<=0){
		...
		float pij = fix32_to_fp32(ptr_pij[index], pij_frac_bit);
		...
		ptr_pij[index] = fp32_to_fix32(pij, pij_frac_bit);
		...
		if(kp){
			float pi = fix32_to_fp32(ptr_pi[i], pi_frac_bit);
			float pj = fix32_to_fp32(ptr_pj[i/dim_conn*dim_mcu + j], pj_frac_bit);
...
void update_j_kernel_cpu(
	...
	float pj = fix32_to_fp32(ptr_pj[idx], pj_frac_bit);
...
/*	if(idx%10==0){
	...
	ptr_pj[idx] = fp32_to_fix32(pj, pj_frac_bit);
...
void update_row_kernel_cpu(
	...
	if(j==0){
		float pi = fix32_to_fp32(ptr_pi[row], pi_frac_bit);
		...
		if(pdt<=0){
			...
			ti = simstep;
			ptr_pi[row] = fp32_to_fix32(pi, pi_frac_bit);
	...
	if(pdt<=0){
		...
		if(kp){
			float pi = fix32_to_fp32(ptr_pi[row], pi_frac_bit);
			float pj = fix32_to_fp32(ptr_pj[idx_mcu], pj_frac_bit);
...
void Proj::update_full_cpu(){
	...
	if(old_prn!=prn){
		fix32 *ptr_pi = _pi->mutable_cpu_data();
		fix16 *ptr_ei = _ei->mutable_cpu_data();
		fix16 *ptr_zi = _zi->mutable_cpu_data();
		int *ptr_ti = _ti->mutable_cpu_data();
		const fix32 *ptr_pj = _pj->cpu_data();
		fix32 *ptr_pij = _pij->mutable_cpu_data();
...
void Proj::update_j_cpu(){
	...
	fix32 *ptr_pj = _pj->mutable_cpu_data();
