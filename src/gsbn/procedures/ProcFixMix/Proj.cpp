...
//	exit(0);*/
	...
	_pi->resize(_dim_hcu * _dim_conn, fp32_to_fix16(pi0, _pi_frac_bit));
	_ei->resize(_dim_hcu * _dim_conn);
	_zi->resize(_dim_hcu * _dim_conn);
	_ti->resize(_dim_hcu * _dim_conn);
	_pj->resize(_dim_hcu * _dim_mcu, fp32_to_fix16(pj0, _pj_frac_bit));
	_ej->resize(_dim_hcu * _dim_mcu);
	_zj->resize(_dim_hcu * _dim_mcu);
	_pij->resize(_dim_hcu * _dim_conn * _dim_mcu, fp32_to_fix16(pij0, _pij_frac_bit));
...
void update_full_kernel_cpu(
	...
	if(j==0){
		float pi = fix16_to_fp32(ptr_pi[i], pi_frac_bit);
		...
		if(pdt<=0){
			...
			ptr_pi[i] = fp32_to_fix16(pi, pi_frac_bit);
	...
	if(pdt<=0){
		...
		float pij = fix16_to_fp32(ptr_pij[index], pij_frac_bit);
		...
		ptr_pij[index] = fp32_to_fix16(pij, pij_frac_bit);
		...
		if(kp){
			float pi = fix16_to_fp32(ptr_pi[i], pi_frac_bit);
			float pj = fix16_to_fp32(ptr_pj[i/dim_conn*dim_mcu + j], pj_frac_bit);
...
void update_j_kernel_cpu(
	...
	float pj = fix16_to_fp32(ptr_pj[idx], pj_frac_bit);
...
/*	if(idx%10==0){
	...
	ptr_pj[idx] = fp32_to_fix16(pj, pj_frac_bit);
...
void update_row_kernel_cpu(
	...
	if(j==0){
		float pi = fix16_to_fp32(ptr_pi[row], pi_frac_bit);
		...
		if(pdt<=0){
			...
			ti = simstep;
			ptr_pi[row] = fp32_to_fix16(pi, pi_frac_bit);
	...
	if(pdt<=0){
		...
		if(kp){
			float pi = fix16_to_fp32(ptr_pi[row], pi_frac_bit);
			float pj = fix16_to_fp32(ptr_pj[idx_mcu], pj_frac_bit);
