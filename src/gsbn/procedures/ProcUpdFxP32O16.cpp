#include "gsbn/procedures/ProcUpdFxP32O16.hpp"

namespace gsbn{
namespace proc_upd_fx_p32_o16{

REGISTERIMPL(ProcUpdFxP32O16);

void ProcUpdFxP32O16::init_new(SolverParam solver_param, Database& db){
	...
	for(int i=0; i<proc_param_size; i++){
		...
		if(proc_param.name()=="ProcUpdFxP32O16"){
			flag=true;
			break;
		}
	}
...
void ProcUpdFxP32O16::init_copy(SolverParam solver_param, Database& db){
...
void ProcUpdFxP32O16::update_cpu(){
...
void ProcUpdFxP32O16::update_gpu(){
	const int* ptr_conf0 = static_cast<const int*>(_conf->cpu_data(0));
	const float* ptr_conf1 = static_cast<const float*>(_conf->cpu_data(0));
	int mode = ptr_conf0[Database::IDX_CONF_MODE];
	if(mode != 1){
