...
void Hcu::update_gpu(){
	...
	// custom kernel
	float* ptr_act_cpu = _act->mutable_cpu_data();
	const float* ptr_dsup_cpu = _dsup->cpu_data();

	float maxdsup = ptr_dsup_cpu[0];
	for(int i=0; i<_mcu_num; i++){
		float dsup=ptr_dsup_cpu[i];
		if(dsup>maxdsup){
			maxdsup=dsup;
		}
	}
	...
	float vsum=0;
	for(int j=0; j<_mcu_num; j++){
		float act = exp(_wtagain*(ptr_dsup_cpu[j]-maxdsup));
		if(maxact<1){
			act *= maxact;
		}
		vsum += act;
