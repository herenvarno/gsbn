...
void Net::init(Database& db){
	...
	CHECK(_rnd_uniform01 = db.table("rnd_uniform01"));
	CHECK(_rnd_normal = db.table("rnd_normal"));
...
void update_kernel_phase_1_cpu(
	...
	const void *ptr_mask, int w_mask,
	...
	float mask = static_cast<const float*>(ptr_mask)[hcu_idx];
	float wgain = ptr_hcu_data0[Database::IDX_HCU_WGAIN]*mask;
...
void Net::update_phase_1(){
	...
	int stim_idx= ptr_conf[Database::IDX_CONF_STIM];
	int mask_idx= ptr_conf[Database::IDX_CONF_GAIN_MASK];
	...
	const void *ptr_stim = _stim->cpu_data(stim_idx);
	int w_mask = _mask->width();
	const void *ptr_mask = _mask->cpu_data(mask_idx);
	int w_rnd_normal = _rnd_normal->width();
	const void *ptr_rnd_normal = _rnd_normal->cpu_data();
...
void Net::update_phase_3(){
	...
	const void *ptr_rnd_uniform01=_rnd_uniform01->cpu_data();
 ...
 */
void update_kernel_phase_5_cpu(
	int idx,
	const void *ptr_mcu, int w_mcu,
	const void *ptr_addr, int w_addr,
	const void *ptr_hcu, int w_hcu,
 ...
 */
void update_kernel_phase_7_cpu(
	int idx,
...
//	const void *ptr_hcu_isp, int w_hcu_isp,
	...
	float dt, float timestamp, float prn, float gain_mask){
 ...
 */
void update_kernel_phase_8_cpu_lazy(
	int idx,
	const void *ptr_mcu, int w_mcu,
	const void *ptr_addr, int w_addr,
	const void *ptr_hcu, int w_hcu,
...
void update_kernel_phase_8_cpu_periodic(
	int idx,
	const void *ptr_mcu, int w_mcu,
	const void *ptr_addr, int w_addr,
	const void *ptr_hcu, int w_hcu,
...
void Net::update_phase_8(){
	...
	/*
	for(int i=0;i<h_mcu;i++){
		update_kernel_phase_8_cpu_lazy(
			i,	// i is the index of "mcu" table
			ptr_mcu, w_mcu,
			ptr_addr, w_addr,
			ptr_hcu, w_hcu,
	...
	for(int i=0;i<h_mcu;i++){
		update_kernel_phase_8_cpu_periodic(
			i,	// i is the index of "mcu" table
			ptr_mcu, w_mcu,
			ptr_addr, w_addr,
			ptr_hcu, w_hcu,
 ...
 */
void update_kernel_phase_9_cpu(
	int idx_tmp1, 
...
/*
 * Phase 10: update ij_mat according to Sj, increase zj2, update incoming spike
 */
void update_kernel_phase_10_cpu(
	...
	const void *ptr_proj, int w_proj,
	void *ptr_ij_mat, int w_ij_mat,
	float dt, float timestamp, float prn){
...
void update_kernel_phase_12_cpu(
	...
	static_cast<float *>(ptr_i_array+i_array_idx*w_i_array)[Database::IDX_I_ARRAY_TI] = 0;
	...
	for(int i=0; i<mcu_num; i++){
		...
		static_cast<float *>(ptr_ij_mat+(ij_mat_idx+i)*w_ij_mat)[Database::IDX_IJ_MAT_TIJ] = 0;
	}
}
...
/*
 * Phase 00: full update if prn changes
 */
void update_kernel_phase_00_cpu(
	int idx,
	const void *ptr_tmp2, int w_tmp2,
	const void *ptr_hcu, int w_hcu,
	const void *ptr_proj, int w_proj,
...
//	int idx_proj = static_cast<const int *>(ptr_hcu_isp+idx_hcu_isp*w_hcu_isp)[Database::IDX_HCU_ISP_VALUE];
    ...
    zi = zi*exp(-kzi*pdt);
		...
		ptr_i_array_data[Database::IDX_I_ARRAY_PI]=pi;
		ptr_i_array_data[Database::IDX_I_ARRAY_EI]=ei;
		ptr_i_array_data[Database::IDX_I_ARRAY_TI]=zi;
		ptr_i_array_data[Database::IDX_I_ARRAY_TI]=ti;
	}
...
void Net::update_phase_00(){
	...
	LOG(INFO) << "PRN changed, fully upate!!";
	old_prn = prn;
	float dt=ptr_conf[Database::IDX_CONF_DT];
	float timestamp=ptr_conf[Database::IDX_CONF_TIMESTAMP];
	...
	_tmp2->reset();
	for(int i=0; i<h_conn; i++){
		const int *ptr_conn = static_cast<const int *>(_conn->cpu_data(i, 0));
		int *ptr_tmp2 = static_cast<int *>(_tmp2->expand(1));
		ptr_tmp2[Database::IDX_TMP2_CONN]=i;
		ptr_tmp2[Database::IDX_TMP2_DEST_HCU]=ptr_conn[Database::IDX_CONN_DEST_HCU];
		ptr_tmp2[Database::IDX_TMP2_SUBPROJ]=ptr_conn[Database::IDX_CONN_SUBPROJ];
