...
Database::Database() : _initialized(false), _tables() {
	...
	_tables["mask"] = new Table("mask", {
		sizeof(float)					// IDX_MASK_VALUE
	});
}
...
void Database::init_new(SolverParam solver_param){
	...
	if (!input) {
		...
		int rows = stim_raw_data.data_rows();
		int cols = stim_raw_data.data_cols();
		...
		rows = stim_raw_data.mask_rows();
		cols = stim_raw_data.mask_cols();
		int mask_size = stim_raw_data.mask_size();
		for(int i=0; i<mask_size; i++){
			float *ptr = static_cast<float *>(_tables["mask"]->expand(1));
			if(ptr){
				ptr[Database::IDX_MASK_VALUE] = stim_raw_data.mask(i);
			}
		}
		height = _tables["mask"]->height();
		if(height!=(cols*rows)){
			LOG(FATAL) << "Invalid stimuli file, bad data!";
		}
		
		_tables["mask"]->set_rows(rows);
		vector<int> v1(cols, sizeof(float));
		_tables["mask"]->set_fields(v1);
	}
...
//	_tables["tmp1"]->expand(_tables["spk"]->height());
	...
	/*
	int h_mcu=_tables["mcu"]->height();
	int h_hcu=_tables["hcu"]->height();
	for(int i=0; i<h_hcu; i++){
		for(int j=0; j<h_mcu; j++){
			int *ptr_conn = static_cast<int*>(_tables["conn"]->expand(1));
			ptr_conn[Database::IDX_CONN_SRC_MCU] = j;
			ptr_conn[Database::IDX_CONN_DEST_HCU] = i;
			ptr_conn[Database::IDX_CONN_SUBPROJ] = 0;
			ptr_conn[Database::IDX_CONN_PROJ] = 0;
			ptr_conn[Database::IDX_CONN_DELAY] = 1;
			ptr_conn[Database::IDX_CONN_QUEUE] = 0;
			ptr_conn[Database::IDX_CONN_TYPE] = 0;
			ptr_conn[Database::IDX_CONN_IJ_MAT_INDEX] = _tables["ij_mat"]->height();
			float *ptr_i_array = static_cast<float*>(_tables["i_array"]->expand(1));
			float pi0 = static_cast<const float *>(_tables["proj"]->cpu_data(0))[Database::IDX_PROJ_PI0];
			int proj_mcu_num = static_cast<const int *>(_tables["proj"]->cpu_data(0))[Database::IDX_PROJ_MCU_NUM];
			ptr_i_array[Database::IDX_I_ARRAY_PI]=pi0;
			ptr_i_array[Database::IDX_I_ARRAY_EI]=0;
			ptr_i_array[Database::IDX_I_ARRAY_ZI]=0;
			ptr_i_array[Database::IDX_I_ARRAY_TI]=0;
			int num_mcu=static_cast<const int *>(_tables["hcu"]->cpu_data(i))[Database::IDX_HCU_MCU_NUM];
			for(int k=0; k<num_mcu; k++){
				float *ptr_wij = static_cast<float*>(_tables["wij"]->expand(1));
				ptr_wij[Database::IDX_WIJ_VALUE]=0;
				float *ptr_ij_mat = static_cast<float*>(_tables["ij_mat"]->expand(1));
				ptr_ij_mat[Database::IDX_IJ_MAT_PIJ] = 1.0/proj_mcu_num/num_mcu;
				ptr_ij_mat[Database::IDX_IJ_MAT_EIJ] = 0;
				ptr_ij_mat[Database::IDX_IJ_MAT_ZI2] = 0;
				ptr_ij_mat[Database::IDX_IJ_MAT_ZJ2] = 0;
				ptr_ij_mat[Database::IDX_IJ_MAT_TIJ] = 0;
			}
		}
	}
	*/
