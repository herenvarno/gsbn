...
if len(sys.argv) < 5:
    ...
    print(
        ...
        + " <network description file> <snapshot file> <projection id> [pij|eij|zi2|zj2|tij|wij]"
    )
    exit(-1)
...
if (
    src_pop_dim_hcu < 0
    or src_pop_dim_mcu < 0
    or dest_pop_dim_hcu < 0
    or dest_pop_dim_mcu < 0
    or dest_pop_dim_conn < 0
):
    print("Error Argument: network description file is wrong!")
    exit(-1)
